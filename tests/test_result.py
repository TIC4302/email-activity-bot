...
''' Logging in. '''
try:
    i.login('tic4302.devops@gmail.com', 'P@ssw0rdNUS')
...
for singlemails in unread_mails:
    ...
    # format goes like this: Create a length which is our predefine queries + 1. 
    # the 0th index stores the setences, the rest of the index stores the predefine queries.
    list_of_responses_keys = list(responses.keys())
    length = len(list_of_responses_keys)+1
    
    # test_data[0] is a sentence we will use to test similiarity with the keys of our dictionary
    test_index = 1
    responses_index = 0

    # here we should are storing a dictionary of  key:sentence, value:array of the different 
    # cos sim comparison 
    results = []
    ...
    for sentence in list_of_sentences:
        ...
        ''' showing what does the test_data variable looks like. '''
        print(test_data)

        # Perform cosine similarity here. 
        # vectorizing test data (consisting of sentence to test and predefine queries)
        # to their vector form for computation of cosine similarity.
        vectors = create_vectorizer(test_data)

        ''' showing what does the vectors variable look like '''
        #print(vectors)

        # 1) for each sentence, compute cos sim using [0] and each subsequent index.
        
        # 2) create a dictionary to store the predefined queries and the cos sim value rel to the        # test setence @ [0].

        # 3) append each result in a tuple format, with [0] index as the sentence we test, 
        # and [1] index as the dictionary we formed in step 2.
